			...
			data = data.filter(el => {
				for (let i = 0; i < this.props.columns.length; i++) {
					...
					if (
						el[key] &&
						el[key].indexOf &&
						el[key].toLowerCase().indexOf(this.state.search) > -1
					) {
						return true;
					}
				}
	...
	getData() {
		...
		if (key.length) {
			return this.state.data.sort((a, b) => {
				...
				if (va < vb) {
					return -1 * this.state.order.direction;
				}
				if (va > vb) {
					return 1 * this.state.order.direction;
				}
	...
	render() {
		return (
			<table className="table-striped">
					...
					<tr>
						{this.props.columns.map(el => (
							<td
								key={el.key || el}
								className={el.key || el}
								onClick={this.orderBy.bind(this, el.key)}
							>
								{el.label || el}
								{el.key === this.state.order.key ? (
									<span
										className={
											this.state.order.direction > 0
												? 'table-triangle-up'
												: 'table-triangle-down'
										}
									/>
				...
				<tbody>{this.elementParser()}</tbody>
			</table>
		);
	}
}
